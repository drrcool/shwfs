...
int main(int argc, char *argv[]) {
  ...
  fov = 1.0;
  mag = atof(argv[3]);
  inner = (argc > 4) ? atof(argv[4]) : 20.0;
  outer = (argc > 5) ? atof(argv[5]) : 30.0;
  fov = outer*2.0/60.0;
  ...
  for (i=0; i<nop; i++) {
	...
	sprintf(dec_dms, "%+03d:%02d:%06.3f", (int)fo_dec, 
      ...
      */
      ...
      printf("%15s  %4.1f mag  %2s  %c  %f  %f  %+07.3f  %+07.2f  %9.2f\n", op[i].co_name, op[i].co_mag/MAGSCALE, op[i].fo_spect, op[i].fo_class, fo_ra, fo_dec, 0.1*op[i].fo_pma/cos(op[i].fo_dec), 0.1*op[i].fo_pmd, dist);
