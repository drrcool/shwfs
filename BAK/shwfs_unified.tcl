...
# look for john's wavedisplay gui locally and run it
if {$fmode == "F5"} {
...
msg_allow WFSSERV {
    128.196.100.50   128.196.100.51
    127.0.0.1        localhost localhost.localdomain
    192.168.1.150    f5wave
    128.196.100.216  wavefront
    192.168.1.200    crunch
    localhost        127.0.0.1
    caliente         caliente.mmto.arizona.edu
    ...
    caliente         caliente.mmto.arizona.edu
    packrat          packrat.mmto.arizona.edu
...
proc show_spot_count {} {

    set spotcount [lindex [eval exec wc -l link] 0]
    #puts "$spotcount spots linked"

    if { $spotcount < 97 } {
	set messg "image;text 300 500 # text={Only $spotcount spots linked, Trouble!!}"
    } else {
	set messg "image;text 250 500 # text={$spotcount spots linked}"
    }

    # typical tcl brain-damage prevents doing the
    # following obvious thing.  Tcl takes the entire
    # first argument (the entire string that follows)
...
proc WFSSERV.centroid { s sock msgid cmd list } {
    ...
    set command "$bin_dir/daofind.py $list $fmode 0"
    puts "centroid routine (MSG) calls: $command"
...
#what actions can and cannot be done. Set initially for all files "*" and no
#actions.
set filter "fits"
...
#Set pointer to the entry widget -- dshow points to the current working
#directory.
...
proc filterFiles {fil} {
    global dshow lbox slv_mode
    $lbox delete 0 end
    cd [$dshow get]
    set files [lsort [glob -nocomplain *.$fil]]
    # check to see which zernike file set to display.
    if {$fil == "ref"} {
	set files [lsort [glob -nocomplain Ref*.fits]]
    }
    if {$slv_mode == 1 && $fil == "zrn"} { 
...
button $AVC.f3.apply -text "Apply" -command {
    filterFiles $filter
    colorButtons  
}
...
proc setpath {dir} {
    global dshow filter 
    $dshow delete 0 end; $dshow insert 0 $dir
    filterFiles $filter
}
...
# But the mode argument is no longer used TJT 1-24-2011
...
proc centroid {mode} { 
	...
	if {[regexp {\.dao} $line]} {
	    ...
	    #now reflush the GUI files listbox
	    filterFiles $filter
	    average $sffile
	} 
    }   
...
proc zernike {avfile} {
    ...
    #Note that slv_mode is appended: 0 = mode solve only (gradients), 1= raw
    #phase solve.
...
button $AVC.f4.centroid -text "Centroid" -command {centroid normal}
...
proc m1correct { } {
	...
	set command "/mmt/bin/rcell -z < zfile 2>1"
	#eval exec $command
	set fid [open |$command r]
	set data [read $fid]
	close $fid
	puts "$data"
	report "* rcell command : $data \n"
...
pack $AVC.f3.name $AVC.f3.fits $AVC.f3.ref $AVC.f3.ctr $AVC.f3.aver $AVC.f3.all \
    $AVC.f3.zrn $AVC.f3.apply -fill x

pack $AVC.f4.name $AVC.f4.centroid $AVC.f4.average $AVC.f4.zernike \
    $AVC.f4.system $AVC.f4.ref 

pack $AVC.f2.files -fill x
