...
class WFSCat
  ...
  def initialize(parent)
    ...
    @glade = GladeXML.new("#{@path}/wfscat/maestro.glade") {|handler| method(handler)}
...
#      puts "maestrostars #{ra} #{dec} #{@mag_entry.text} #{@inner_entry.text} #{@outer_entry.text}"
      result = `#{@path}/wfscat/maestrostars #{ra} #{dec} #{@mag_entry.text} #{@inner_entry.text} #{@outer_entry.text} | grep S | sort -n -k11`
	...
	loop {
	  ...
	  result.each_line { |star|
	    ...
	    offset = @flip-38.0
	    pa = `#{@path}/wfscat/bear.tcl #{ra} #{dec} #{data[6]} #{data[7]}`.to_f - offset
	    rot = parang - pa
	    if rot > 180.0
	       rot = 360.0 - rot
        end
