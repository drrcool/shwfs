...
def daofind(im):
    ...
    if mode == 'F9':
        ...
        smooth = nd.gaussian_filter(im, 1.15)
        nsig = 8.0
        maxstars = 155
...
def shcenfind(fitsfile, mode, xcen, ycen):
    ...
    oldref = open(ref[mode], 'r')
    oldreflines = oldref.readlines()
    oldref.close()
    (dum1, dum2, xrmag, yrmag, xrc, yrc) = oldreflines[1].split(' ')
    xrc = float(xrc)
    yrc = float(yrc)
    ...
    refdat = pylab.load(ref[mode])
    star = pylab.load(daofile)
    guess = array([-20,0])
    fit = optimize.fmin_powell(pattern_dist, guess, args=(refdat,star), xtol=0.00001, disp=False)
    ...
    xrc = xrc + fit[0]
    yrc = yrc + fit[1]
    newref = open('ref.dat', 'w')
    newref.write("# reference for %s\n" % fitsfile)    
    newref.write("# X %s %s %f %f\n" % (xrmag, yrmag, xrc, yrc))
    for i in refdat:
        newref.write("%f %f\n" % (i[0], i[1]))
    newref.flush()
    newref.close()
...
ref['MMIRS'] = "/mmt/shwfs/mmirs_wfsc2_sysfile.cntr"
