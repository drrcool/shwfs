 ...
 */
 ...
 main (int argc, char *argv[])
 {
   ...
   const int RPUP = 32;	/* resolution elements along pupil radius */
   const int SZ = 2*RPUP*2*RPUP; /* size for 1D vectors to hold 2D data */
   const int det_size = 32;	/* # image pixels in x and y */
   ...
   if (getenv("WFSROOT")) {
     ...
     apts_cntr = (char *) malloc(strlen(wfsroot) + 10);
     strcpy(apts_cntr, wfsroot);
     strcat(apts_cntr, "/blah.apts");
   } else {
     apts_cntr = "/mmt/shwfs/blah.apts";
   }
   ...
   psf (FL, ds, 1.0, napts, &hphs[1], &hx[1], &hy[1], Npixels, &detI[1],
	&detx[1], &dety[1]);
