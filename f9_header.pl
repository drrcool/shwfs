...
# now hexapod....
$hex_socket = IO::Socket::INET->new(PeerAddr => "hacksaw",
				    PeerPort => 5350,
				    Proto => "tcp",
				    Type => SOCK_STREAM)
...
$img->fhdr->{'CROTA2'} = 180 + (-225 + $img->fhdr->{'ROT'});
$img->fhdr->{'CROTA2_COMMENT'} = "Sky Angle";

# add SECZ header
$img->fhdr->{'SECZ'} = $img->fhdr->{'AIRMASS'};

# set up the fitsheader comments
...
$img->fhdr->{'CURRTEMP_COMMENT'} = "OSS Temperature of observation";
