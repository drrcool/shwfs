...
def daofind(im):
    # near as I can tell this center info is never used.
...
def fits2mysql(fits_filename, mode, psf, see_as):
    # Read in the FITS header parameters that we need.
    try:
        # Open the FITS file, argv[1], and read header information.
        hdulist = open(fits_file)
        # Date of observation, e.g., 2011-04-23
        d = hdulist[0].header['DATE-OBS']
        # UT for observation (not localtime...), e.g., 02:38:22
        time = hdulist[0].header['TIME-OBS']
        # Exposure time (in seconds)
        exptime = hdulist[0].header['EXPTIME']
        # azimuth (in decimal degrees)
        az = hdulist[0].header['AZ']
        # elevation (in decimal degrees)
        el = hdulist[0].header['EL']
        # rotator angle (in decimal degrees)
        rot = hdulist[0].header['ROT']
    ...
    zenith_angle_rads = ((90 - el)/180.0) * 3.14159265358979;
    exponent = 0.6;
    see_zenith_as = float(see_as) * pow( cos(zenith_angle_rads), exponent );
        
    # Connect to mmtlogs:wfs_seeing_log and insert new seeing entry.
    try:
        database = 'mmtlogs';
        ...
        query += "DATE_SUB('" + timestamp + "', INTERVAL 7 HOUR), "
        query += str(exptime) + ", "
        query += str(az) + ", "
        query += str(el) + ", "
        query += str(rot) + ", "       
